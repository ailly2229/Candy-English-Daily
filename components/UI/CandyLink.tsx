...
export function CandyLink({ className = "", tone = "pink", ...props }: CandyLinkProps) {
  return (
    <Link
      className={`candy-button inline-flex min-h-11 items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold ${tones[tone]} ${className}`}
      {...props}
    />
  );
}
